...
function theme_fordson_get_main_scss_content($theme) {
    ...
    if (!$presetisset) {
        ...
        if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_fordson', 'preset', 0, '/', $filename))) {
            ...
            $scss .= file_get_contents($CFG->dirroot . '/theme/fordson/scss/fordson.scss');
        }
    }
...
function theme_fordson_get_pre_scss($theme) {
    ...
    // Config key => variableName,
		...
		'fpicon' => ['fpicon'],
		'fpiconhover' => ['fpicon-hover-c'],
		'headercoursecolor' => ['header-course-color'],
		'blockbg' => ['block-bg'],
		'blockheader' => ['block-header'],
		'blockheaderbg' => ['block-header-bg'],
		'breadcrumbcolor' => ['breadcrumb-color'],
		'topicheaderbg' => ['topic-header-bg'],
